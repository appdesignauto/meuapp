#!/bin/bash

# Script para testar o endpoint aprimorado de webhook da Hotmart
echo "🧪 Iniciando teste do webhook aprimorado..."

# Payload do webhook Hotmart - mesmo formato que falhou anteriormente
PAYLOAD='{
  "id": "42338ce6-7974-4519-854a-cae504d820a1",
  "creation_date": 1747540995368,
  "event": "PURCHASE_APPROVED",
  "version": "2.0.0",
  "data": {
    "product": {
      "id": 5381714,
      "ucode": "eef7f8c8-ce74-47aa-a8da-d1e7505dfa8a",
      "name": "App DesignAuto",
      "warranty_date": "2025-05-25T00:00:00Z",
      "has_co_production": false,
      "is_physical_product": false
    },
    "affiliates": [
      {
        "affiliate_code": "",
        "name": ""
      }
    ],
    "buyer": {
      "email": "canvaparamusicos@gmail.com",
      "name": "Fernando Oliveira ",
      "first_name": "Fernando",
      "last_name": "Oliveira",
      "address": {
        "country": "Brasil",
        "country_iso": "BR"
      },
      "document": "",
      "document_type": ""
    },
    "producer": {
      "name": "EDITORA INOVE DIGITAL LTDA",
      "document": "52883206000100",
      "legal_nature": "Pessoa Jurídica"
    },
    "commissions": [
      {
        "value": 1.4,
        "source": "MARKETPLACE",
        "currency_value": "BRL"
      },
      {
        "value": 5.6,
        "source": "PRODUCER",
        "currency_value": "BRL"
      }
    ],
    "purchase": {
      "approved_date": 1747540992000,
      "full_price": {
        "value": 7,
        "currency_value": "BRL"
      },
      "price": {
        "value": 7,
        "currency_value": "BRL"
      },
      "checkout_country": {
        "name": "Brasil",
        "iso": "BR"
      },
      "order_bump": {
        "is_order_bump": false
      },
      "original_offer_price": {
        "value": 7,
        "currency_value": "BRL"
      },
      "order_date": 1747540956000,
      "status": "APPROVED",
      "transaction": "HP1787159814",
      "payment": {
        "installments_number": 1,
        "type": "PIX",
        "pix_qrcode": "https://test-url/qrcode",
        "pix_code": "00020101021226900014br.gov.bcb.pix2568pix.test",
        "pix_expiration_date": 1747713756000
      },
      "offer": {
        "code": "aukjngrt"
      },
      "invoice_by": "HOTMART",
      "subscription_anticipation_purchase": false,
      "date_next_charge": 1779105600000,
      "recurrence_number": 1,
      "is_funnel": false,
      "business_model": "I"
    },
    "subscription": {
      "status": "ACTIVE",
      "plan": {
        "id": 1038897,
        "name": "Plano Anual"
      },
      "subscriber": {
        "code": "9OL3KY4O"
      }
    }
  },
  "hottok": "azjZzEUU43jb4zN4NqEUrvRu1MO1XQ1167719"
}'

# URL do endpoint de webhook aprimorado
WEBHOOK_URL="http://localhost:5000/webhook/hotmart-enhanced"

echo "📤 Enviando webhook para o endpoint aprimorado..."
echo "URL: $WEBHOOK_URL"

# Enviar requisição usando cURL
curl -X POST \
  -H "Content-Type: application/json" \
  -H "X-Hotmart-Webhook-Token: teste-de-assinatura" \
  -H "User-Agent: Hotmart-Webhooks/1.0" \
  -d "$PAYLOAD" \
  "$WEBHOOK_URL"

echo -e "\n\n✅ Webhook enviado. Verifique os logs do servidor para mais detalhes."