...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // ========================================
  // SOCIAL GROWTH API ENDPOINTS - Following PRD
  // ========================================
  ...
  app.get("/api/social/dashboard", isAuthenticated, async (req, res) => {
    try {
      ...
      const alerts = await db
        .select()
        .from(socialAlerts)
        .where(and(eq(socialAlerts.userId, userId), eq(socialAlerts.isRead, false)))
        .orderBy(desc(socialAlerts.createdAt))
        .limit(5);

      // Calcular métricas de crescimento atual vs mês anterior
      const currentMonth = new Date().toISOString().slice(0, 7); // 2025-06
      const lastMonth = new Date(new Date().setMonth(new Date().getMonth() - 1)).toISOString().slice(0, 7);

      // Buscar dados de progresso para cálculos de crescimento
      const currentProgress = await db
        .select()
        .from(socialProgress)
        .where(and(eq(socialProgress.userId, userId), eq(socialProgress.monthYear, currentMonth)));

      const lastProgress = await db
        .select()
        .from(socialProgress)
        .where(and(eq(socialProgress.userId, userId), eq(socialProgress.monthYear, lastMonth)));

      // Calcular totais de seguidores e vendas
      const totalFollowers = currentProgress
        .filter(p => p.metricType === 'followers')
        .reduce((sum, p) => sum + p.value, 0);

      const totalSales = currentProgress
        .filter(p => p.metricType === 'sales')
        .reduce((sum, p) => sum + p.value, 0);

      const lastMonthFollowers = lastProgress
        .filter(p => p.metricType === 'followers')
        .reduce((sum, p) => sum + p.value, 0);

      const lastMonthSales = lastProgress
        .filter(p => p.metricType === 'sales')
        .reduce((sum, p) => sum + p.value, 0);

      // Calcular crescimento percentual
      const followersGrowth = lastMonthFollowers > 0 
        ? Math.round(((totalFollowers - lastMonthFollowers) / lastMonthFollowers) * 100)
        : 0;
      ...
      // Buscar dados para gráficos de tendência (últimos 6 meses)
      ...
      const trendData = await db
        ...
        .where(and(
          ...
          gte(socialProgress.monthYear, sixMonthsAgo.toISOString().slice(0, 7))
        ))
        .orderBy(socialProgress.monthYear);

      // Formatar dados de tendência para gráficos
      ...
      for (let i = 0; i < 6; i++) {
        ...
        const monthData = trendData.filter(d => d.monthYear === monthStr);
        const followersValue = monthData.filter(d => d.metricType === 'followers').reduce((sum, d) => sum + d.value, 0);
        const salesValue = monthData.filter(d => d.metricType === 'sales').reduce((sum, d) => sum + d.value, 0);
        
        followersTrend.push({ month: monthStr, value: followersValue });
        salesTrend.push({ month: monthStr, value: salesValue });
      }

      // Calcular progresso das metas
      const goalsProgress = goals.map(goal => {
        const currentValue = currentProgress
          .filter(p => p.platform === goal.platform && p.metricType === goal.goalType)
          .reduce((sum, p) => sum + p.value, 0);
        ...
        else if (percentage >= timeProgress * 0.8) status = 'on_track';
        else if (percentage >= timeProgress * 0.6) status = 'at_risk';
        else status = 'behind';
      ...
      console.error('Erro no dashboard social:', error);
  ...
  app.get("/api/social/progress", isAuthenticated, async (req, res) => {
    try {
      ...
      if (platform) {
        query = query.where(eq(socialProgress.platform, platform as string));
      }
      if (metricType) {
        query = query.where(eq(socialProgress.metricType, metricType as string));
      }
      if (monthYear) {
        query = query.where(eq(socialProgress.monthYear, monthYear as string));
      }

      const progress = await query.orderBy(desc(socialProgress.monthYear));
  ...
  app.post("/api/social/progress", isAuthenticated, async (req, res) => {
    try {
      ...
      // Verificar se já existe registro para este mês/plataforma/métrica
      const [existing] = await db
        ...
        .where(and(
          ...
          eq(socialProgress.metricType, progressData.metricType),
          eq(socialProgress.monthYear, progressData.monthYear)
        ));
      ...
      if (existing) {
        ...
        const [updated] = await db
          ...
          .set({
            ...
            growthRate: progressData.growthRate,
  ...
  app.get("/api/social/alerts", isAuthenticated, async (req, res) => {
    try {
      ...
      const alerts = await db
        .select()
        .from(socialAlerts)
        .where(and(eq(socialAlerts.userId, userId), eq(socialAlerts.isDismissed, false)))
        .orderBy(desc(socialAlerts.createdAt));
  ...
  app.put("/api/social/alerts/:id/read", isAuthenticated, async (req, res) => {
    try {
      ...
      await db
        .update(socialAlerts)
        .set({ isRead: true })
        .where(and(eq(socialAlerts.id, alertId), eq(socialAlerts.userId, userId)));
