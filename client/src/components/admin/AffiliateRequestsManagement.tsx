...
import { Award, Eye, MessageSquare, Trash2, Calendar, CheckCircle, XCircle, Clock, Mail, Phone, ExternalLink, Users } from 'lucide-react';
...
const AffiliateRequestsManagement = () => {
  const [activeTab, setActiveTab] = useState<string>('pending');
  const [currentPage, setCurrentPage] = useState(1);
  const [searchTerm, setSearchTerm] = useState('');
  const [currentRequest, setCurrentRequest] = useState<AffiliateRequest | null>(null);
  ...
  // Query para buscar solicitações de afiliação
  const { 
    data: requestsData,
    isLoading: isLoadingRequests,
    isError: isRequestsError
  } = useQuery({
    queryKey: ['/api/admin/affiliate-requests', activeTab, searchTerm],
    queryFn: async () => {
      ...
      const response = await apiRequest('GET', `/api/admin/affiliate-requests?${queryParams}`);
  ...
  const updateRequestMutation = useMutation({
    mutationFn: async ({ id, status, adminNotes }: { id: number; status: string; adminNotes?: string }) => {
      const response = await apiRequest('PATCH', `/api/admin/affiliate-requests/${id}`, {
        status,
        adminNotes
      });
  ...
  const deleteRequestMutation = useMutation({
    mutationFn: async (id: number) => {
      const response = await apiRequest('DELETE', `/api/admin/affiliate-requests/${id}`);
  ...
  const openRequestDetails = (request: AffiliateRequest) => {
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div>
          <h1 className="text-3xl font-bold tracking-tight">Solicitações de Afiliação</h1>
          <p className="text-muted-foreground">
            Gerencie solicitações do programa de afiliados
          </p>
        </div>
      </div>
      ...
      <div className="flex flex-col sm:flex-row gap-4">
        <div className="flex-1">
          <Input
            placeholder="Buscar por nome, email, empresa ou nicho..."
            value={searchTerm}
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab}>
        ...
        <TabsContent value={activeTab} className="space-y-4">
          {isLoadingRequests ? (
            ...
            <>
              {paginatedRequests.map((request: AffiliateRequest) => (
                      ...
                      <div className="flex gap-2">
                        {request.website && (
                          <Button
                            variant="outline"
                            size="sm"
                            onClick={() => window.open(request.website, '_blank')}
                          >
        ...
        <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2">
              ...
              Detalhes da Solicitação de Afiliação
            </DialogTitle>
            <DialogDescription>
              Gerencie esta solicitação do programa de afiliados
            </DialogDescription>
          </DialogHeader>
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                {currentRequest.company && (
                  <div>
                    ...
                    <p className="text-sm mt-1">{currentRequest.company}</p>
                  </div>
                )}
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  ...
                  <p className="text-sm mt-1 bg-gray-50 p-3 rounded-md">{currentRequest.niche}</p>
                </div>
                <div>
              ...
              <div>
                <Label className="text-sm font-medium">Estratégia de Promoção</Label>
                <p className="text-sm mt-1 bg-gray-50 p-3 rounded-md">{currentRequest.promotionStrategy}</p>
              </div>
                ...
                {currentRequest.website && (
                  <div>
                    ...
                    <Button
                      variant="outline"
                      size="sm"
                      onClick={() => window.open(currentRequest.website, '_blank')}
                      className="ml-2"
                    >
                      <ExternalLink className="h-4 w-4 mr-1" />
                      Visitar Website
...
export default AffiliateRequestsManagement;