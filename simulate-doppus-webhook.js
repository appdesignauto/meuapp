...
// Aguardar um pouco e fazer um segundo teste com status diferente
setTimeout(() => {
  console.log('\n🔄 Testando webhook com status de cancelamento...');
  
  const cancelPayload = {
    ...webhookPayload,
    id: "txn_test_cancel_54321",
    status: {
      code: "canceled",
  ...
  const data = JSON.stringify(cancelPayload);
  const url = new URL(WEBHOOK_URL);
  const isHttps = url.protocol === 'https:';
  const client = isHttps ? https : http;
  ...
  const options = {
    ...
    headers: {
      ...
      'Content-Length': data.length,
      'User-Agent': 'Doppus-Webhook/1.0',
      'X-Doppus-Event': 'transaction.canceled'
    }
  };

  const req = client.request(options, (res) => {
    console.log(`📈 Status da resposta (cancelamento): ${res.statusCode}`);
    ...
    res.on('end', () => {
      try {
        const jsonResponse = JSON.parse(responseData);
        console.log('📦 Resposta do teste de cancelamento:');
        console.log(JSON.stringify(jsonResponse, null, 2));
      } catch (error) {
        console.log('📄 Resposta (texto):', responseData);
      }
  ...
  req.on('error', (error) => {
    console.error('❌ Erro no teste de cancelamento:', error.message);
  });
